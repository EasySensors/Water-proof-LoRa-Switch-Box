...
int relayNodeID[8] = {0, 0, 0, 0, 0, 0, 0, 0}; // Relay addressess for reach button to send switch ON\OFF states. Can be any address; 0 is SmartHome controller address.
int relayChildID[8] = {1, 2, 3, 4, 5, 6, 7, 8}; //NULL value means no need to report\present it to cntroller;
#define NUMBER_OF_BUTTONS 8
int switchButtonPin[8] = {8, A1, 7, A0, 6, A7, 5, A2};  //Arduino Pins A1 = 15 A0 = 14 A7 = 22 A2 = 16
int switchButtonLeds[8] = {4, 4, 4, 4, 4, 4, 4, 4}; // LED connected Arduino Pins  
...
void loop(){ 
  ...
  for (int i = 0; i < NUMBER_OF_BUTTONS; i++) {
    Serial.print("BTN");
    Serial.println( digitalRead(switchButtonPin[i])  );
    if ((digitalRead(switchButtonPin[i]) == 0)) {
